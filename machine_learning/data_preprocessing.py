"""
Makine Öğrenmesş veri ön işleme praikleri

Amaç: 
    1. Eksik veri tespiti, çıkarılması ve uygun değerler ile doldurma
    2. IQR yöntemi ile sayısal sütunlarda ki aykırı değerleri tespit et
    3. Kategorik veileri label encoding ve one-hot encoding ile dönüştür
    4. Veriyi train, validasyon ve test kümelerine ayır
    5. Sayısal özelliklere standirzation ve normalization uygula
"""
...
df = pd.read_csv("C:\\Users\\ahmet\\Desktop\\bootcamp\\machine_learning\\musteri_verisi_ml_pratik.csv")
print(df.head())
print(df.info())
...
for sutun in sayisal_sutunlar:
  median_deger = df_filled[sutun].median()
  df_filled[sutun] = df_filled[sutun].fillna(median_deger)
...
for sutun in sayisal_sutunlar:
    ...
    sutun_maskesi = (
       (df_filled[sutun] < alt_sinir) | 
       (df_filled[sutun] > ust_sinir)

    )

    aykiri_deger_maskesi = aykiri_deger_maskesi | sutun_maskesi #eklemek demek

    print(f"Aykırı deger sayısı : {sutun_maskesi.sum()}")

    if sutun_maskesi.any():
       print(f"Aykırı degerler : \n {df_filled.loc[sutun_maskesi , sutun]}")
...
print(f"Aykırı degerler cıktıktan sonra : \n {df_clean}")
...
# Ölçcekleyiciyi yalnıza eğitim veri seti üzerinde öğretiyoruz
...
# Validasyon ve test verilerinde yalnızca transforma uygula
